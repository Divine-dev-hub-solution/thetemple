...
if( !maharatri_is_yith_wcqv_active() ){
  return;
}
...
add_action('maharatri/product/controls', 'maharatri_yith_quickview_print', 10);
...
add_action('yith_wcqv_product_summary', 'maharatri_product_stock_status', 30);
 ...
 * @since 1.0.0
 */
function maharatri_get_yith_quickview_link(){
  global $product;
  $button = '<a href="#" title="'.esc_attr__('Quick View', 'maharatri').'" class="yith-wcqv-button" data-product_id="' . esc_attr( $product->get_id() ) . '" data-toggle="tooltip"><i class="far fa-eye"></i> </a>';
 ...
 * @since 1.0.0
 */
function maharatri_yith_quickview_print(){
  $show_wcqv_in_loop = get_option('yith-wcqv-enable');
  echo (!empty($show_wcqv_in_loop) && $show_wcqv_in_loop == 'yes' && maharatri_is_yith_wcqv_active() ) ? maharatri_get_yith_quickview_link() : '';
}
