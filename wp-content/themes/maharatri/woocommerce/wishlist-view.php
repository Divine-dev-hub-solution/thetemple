...
<!-- WISHLIST TABLE -->
<table
	class="shop_table cart wishlist_table wishlist_view traditional responsive <?php echo !empty($no_interactions) ? esc_attr('no-interactions') : ''; ?> <?php echo !empty($enable_drag_n_drop) ? esc_attr('sortable') : ''; ?> "
	...
	<tr>
		...
		<th class="product-name">
			<span class="nobr">
				<?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product', 'maharatri' ), $wishlist ); ?>
			</span>
		</th>
		<?php if ( $show_price || $show_price_variations ) : ?>
			...
			<th class="product-price">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Price', 'maharatri' ), $wishlist ) ; ?>
				</span>
			</th>
		<?php endif; ?>
		<?php if ( $show_quantity ) : ?>
			...
			<th class="product-quantity">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_quantity_heading', esc_html__( 'Quantity', 'maharatri' ), $wishlist ) ; ?>
				</span>
			</th>
		<?php endif; ?>
		<?php if ( $show_stock_status ) : ?>
			...
			<th class="product-stock-status">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock status', 'maharatri' ), $wishlist ); ?>
				</span>
			</th>
		<?php endif; ?>
		<?php if ( $show_last_column ) : ?>
			...
			<th class="product-add-to-cart">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_cart_heading', esc_html__( 'Actions', 'maharatri' ), $wishlist ); ?>
				</span>
			</th>
		<?php endif; ?>
		<?php if ( $enable_drag_n_drop ) : ?>
			...
			<th class="product-arrange">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_arrange_heading', esc_html__( 'Arrange', 'maharatri' ), $wishlist ); ?>
				</span>
			</th>
		<?php endif; ?>
	</tr>
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<?php if ( $show_remove_product ) : ?>
						<td class="product-remove">
							<div>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove close-btn close-danger remove_from_wishlist" title="<?php echo  apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'maharatri' ) ); ?>">
                  <span></span>
                  <span></span>
                </a>
							</div>
            ...
            <?php do_action( 'yith_wcwl_table_before_product_name', $item, $wishlist ); ?>
					...
					<?php if ( $show_stock_status ) : ?>
						<td class="product-stock-status">
							...
							<?php echo 'out-of-stock' === $stock_status ? '<span class="wishlist-out-of-stock">' . apply_filters( 'yith_wcwl_out_of_stock_label', esc_html__( 'Out of stock', 'maharatri' ) ) . '</span>' : '<span class="wishlist-in-stock">' . apply_filters( 'yith_wcwl_in_stock_label', esc_html__( 'In Stock', 'maharatri' ) ) . '</span>'; ?>
					...
					<?php if ( $show_last_column ) : ?>
						<td class="product-add-to-cart">
							...
							if ( $show_dateadded && $item->get_date_added() ) :
								...
								echo '<span class="dateadded">' . sprintf( esc_html( 'Added on: %s', 'maharatri' ), $item->get_date_added_formatted() ) . '</span>';
							endif;
							?>
							...
							<?php if ( $move_to_another_wishlist && $available_multi_wishlist && count( $users_wishlists ) > 1 ) : ?>
								<?php if ( 'select' === $move_to_another_wishlist_type ) : ?>
									<select class="change-wishlist selectBox">
										<option value=""><?php esc_html_e( 'Move', 'maharatri' ); ?></option>
										<?php
								...
								<?php else : ?>
									<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
										<?php echo apply_filters( 'yith_wcwl_move_to_another_list_label', esc_html__( 'Move to another list &rsaquo;', 'maharatri' ) ); ?>
									</a>
								<?php endif; ?>
							...
							<?php if ( $repeat_remove_button ) : ?>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove_from_wishlist button" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'maharatri' ) ); ?>"><?php esc_html_e( 'Remove', 'maharatri' ); ?></a>
							<?php endif; ?>
	...
	?>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist', 'maharatri' ), $wishlist ); ?></td>
		</tr>
	<?php
	endif;
	if ( ! empty( $page_links ) ) :
