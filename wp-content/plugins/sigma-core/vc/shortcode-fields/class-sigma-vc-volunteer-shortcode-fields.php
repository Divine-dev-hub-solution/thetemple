 ...
 * Volunteer shortcode.
 */
class Sigma_Vc_Volunteer_Shortcode_Fields {
	...
	function __construct() {
		...
		$this->title         = esc_html__( 'Volunteer', 'sigma-core' );
		$this->handle        = 'sigma_volunteer';
		$this->description   = esc_html__( 'Use this shortcode to show volunteer.', 'sigma-core' );
	...
	function shortcode_fields() {
		...
		$fields = array(
			...
			array(
				'type'        => 'dropdown',
				'heading'     => __( 'Volunteer Style', 'sigma-core' ),
			...
			array(
				'type'        => 'textfield',
				'heading'     => esc_html__( 'Volunteer Count', 'sigma-core' ),
				'param_name'  => 'post_per_page',
				'description' => esc_html__( 'Enter number of volunteer to display.', 'sigma-core' ),
				'admin_label' => true
			),
			array(
				'type'        => 'checkbox',
	...
	/**
	 * Volunteer shortcode callback functions.
	 */
	function shortcode_callback( $atts, $content = null, $handle = '' ) {
		...
		$sigma_shortcodes_class_unique              = 'sigma-volunteer-' . mt_rand();
		...
		$sigma_shortcodes_classes                   .= ' sigma-volunteer-' . $atts[ 'style' ];
		$sigma_shortcodes_classes                   .= ' volunteer-layout-' . $atts[ 'layout' ];
		...
		<div <?php echo ( $atts[ 'el_id' ] ) ? 'id=' . esc_attr( $atts[ "el_id" ] ) : ''; ?> class="<?php echo esc_attr( $sigma_shortcodes_classes ); ?>">
			<?php sigmacore_get_vc_shortcode_template( 'volunteer/content' ); ?>
		</div>
		<?php
		return ob_get_clean();
	}
...
new Sigma_Vc_Volunteer_Shortcode_Fields();
