<?php
/**
 * Sigma Device images shortcode
 *
 * @package Sigma Core
 */

/**
 * Device images shortcode.
 */
class Sigma_Vc_Device_Images_Shortcode_Fields {
	...
	function __construct() {
		...
		$this->title         = esc_html__( 'Device Images', 'sigma-core' );
		$this->handle        = 'sigma_device_images';
		$this->description   = esc_html__( 'Use this shortcode to show image into device.', 'sigma-core' );
	...
	function shortcode_fields() {
        $fields = array(
            array(
				'type'             => 'dropdown',
				'param_name'       => 'layout',
				'heading'          => esc_html__( 'Device Layout', 'sigma-core' ),
				'description'      => esc_html__( 'Select Device layout.', 'sigma-core' ),
				'std'              => 'laptop',
				'value'            => array(
					esc_html__( 'Laptop', 'sigma-core' ) => 'laptop',
					esc_html__( 'Laptop with Smartphone', 'sigma-core' )   => 'laptop-smartphone',
				),
			),
        	array(
				'type'        => 'attach_image',
				'heading'     => esc_html__( 'Image for Laptop', 'sigma-core' ),
				'param_name'  => 'laptop_image',
				'value'       => '',
				'description' => esc_html__( 'Add an image to display inside the laptop.', 'sigma-core' ),
				'group'       => esc_html__( 'Image Options', 'sigma-core' ),
            ),
            array(
				'type'        => 'attach_image',
				'heading'     => esc_html__( 'Image for Smartphone', 'sigma-core' ),
				'param_name'  => 'smartphone_image',
				'value'       => '',
				'description' => esc_html__( 'Add an image to display inside the smartphone.', 'sigma-core' ),
                'group'       => esc_html__( 'Image Options', 'sigma-core' ),
                'dependency' => array(
					'element' => 'layout',
	...
	function shortcode_callback( $atts, $content = null, $handle = '' ) {
		...
		$sigma_shortcodes_class_unique       = 'sigma-device-images-' . mt_rand();
        ...
        $sigma_shortcodes_classes .= ' device-images-layout-' . $atts['layout'];
		...
		<div <?php echo ( $atts[ 'el_id' ] ) ? 'id=' . esc_attr( $atts[ "el_id" ] ) : ''; ?> class="<?php echo esc_attr( $sigma_shortcodes_classes ); ?>">
			<?php sigmacore_get_vc_shortcode_template( 'device-images/content' ); ?>
		</div>
		<?php
		return ob_get_clean();
	}
...
new Sigma_Vc_Device_Images_Shortcode_Fields();
