 ...
 * List shortcode.
 */
class Sigma_Vc_List_Shortcode_Fields {
	/**
	 * Shortcode title.
	 *
	 * @since 1.0.0
	...
	function __construct() {
		$this->title         = esc_html__( 'List', 'sigma-core' );
		$this->handle        = 'sigma_list';
		$this->description   = esc_html__( 'Use this shortcode to show list.', 'sigma-core' );
		...
		$fields = array(
			...
			array(
				...
				'params'     => array(
					...
					array(
						'type'        => 'vc_link',
						'heading'     => esc_html__( 'Title Link', 'sigma-core' ),
	...
	/**
	 * List shortcode callback functions.
	 */
	function shortcode_callback( $atts, $content = null, $handle = '' ) {
		...
		$sigma_shortcodes_class_unique       = 'sigma-list-' . mt_rand();
		...
		<div <?php echo ( $atts[ 'el_id' ] ) ? 'id=' . esc_attr( $atts[ "el_id" ] ) : ''; ?> class="<?php echo esc_attr( $sigma_shortcodes_classes ); ?>">
			<?php sigmacore_get_vc_shortcode_template( 'list/content' ); ?>
		</div>
		<?php
		return ob_get_clean();
	}
}
new Sigma_Vc_List_Shortcode_Fields();
