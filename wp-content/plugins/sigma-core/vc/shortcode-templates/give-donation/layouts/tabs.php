<?php
/**
 * Give Donation shortcode tabs layout file.
 *
 * @package sigma Core
 */
...
$the_query   = $sigma_shortcodes[ 'sigma_give_donation' ][ 'the_query' ];
$tabs_category   = $sigma_shortcodes[ 'sigma_give_donation' ][ 'tabs_category' ];
$tab_style = isset($atts['tabs_style']) ? $atts['tabs_style'] : 'style-1';
?>
<?php if($tab_style == 'style-5') { ?>
	<div class="flex-section">
	<?php if($atts['section_title'] || $atts['section_subtitle']) { ?>
		<div class="custom-heading-style-2">
			<div class="section-title sigma-custom-heading-wrapper">
				<p class="heading-subtitle"><?php echo esc_html($atts['section_subtitle']); ?></p>
				<h4 class="heading-title"><?php echo esc_html($atts['section_title']); ?></h4>
			</div>
		</div>
<?php } } ?>
<div class="text-center sigma_post-filter <?php echo esc_attr('tabs-'.$tab_style); ?>">
	...
	<?php
			...
			$posts = get_posts(["post_type" => "give_forms", "cat" => $term->term_id]);
	?>
...
<div class="row sigma_post-filter-items">
			...
			$args = array(
				...
				if ($loop->have_posts()) {
						while ($loop->have_posts()) {
								...
								$terms = wp_get_object_terms( get_the_ID(), 'give_forms_category', array( 'fields' => 'slugs' ) );
	...
	<div class="col-lg-4 grid-item grid-sizer <?php echo esc_attr($term_names); ?>">
	 <?php sigmacore_get_vc_shortcode_template( 'give-donation/styles/' . $atts[ 'style' ] ); ?>
	</div>
	<?php }
		} // end if
			wp_reset_postdata();
