<?php
/**
 * blog shortcode tabs layout file.
 *
 * @package sigma Core
 */
...
$tabs_category   = $sigma_shortcodes[ 'sigma_blog' ][ 'tabs_category' ];
...
<div class="text-center sigma_post-filter <?php echo esc_attr('tabs-'.$tab_style); ?>">
	...
	<?php
			...
			$posts = get_posts(["post_type" => "post", "cat" => $term->term_id]);
	?>
			...
			$args = array(
				...
				if ($loop->have_posts()) {
						while ($loop->have_posts()) {
								...
								$terms = wp_get_object_terms( get_the_ID(), 'category', array( 'fields' => 'slugs' ) );
								$term_names = !empty($terms) ? implode($terms, " ") : '';
								?>

	<div class="col-lg-4 grid-item grid-sizer <?php echo esc_attr($term_names); ?>">
	 <?php sigmacore_get_vc_shortcode_template( 'blog/styles/' . $atts[ 'style' ] ); ?>
	</div>
	<?php }
		} // end if
			wp_reset_postdata();
