...
<?php foreach($history_lists as $history_list) {
  $text_alignment = isset($history_list['content_alignment']) ? $history_list['content_alignment'] : 'align-left';
  ?>
  <div class="row no-gutters justify-content-end justify-content-md-around align-items-start sigma_timeline-nodes <?php echo esc_attr($text_alignment); ?>">
    ...
    <div class="col-12 col-md-5 order-1 order-md-3 pb-3 sigma_timeline-date">
      <?php if(isset($history_list['history_date']) && !empty($history_list['history_date'])) { ?>
        <span><?php echo html_entity_decode(vc_value_from_safe($history_list['history_date'], true)); ?></span>
      <?php } ?>
    </div>
  </div>
<?php } ?>
