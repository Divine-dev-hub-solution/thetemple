<?php
/**
 * video shortcode style 3 template file.
 *
 * @package sigma Core
 */
if ( ! defined( 'ABSPATH' ) ) {
...
$icon_skin = isset($atts['icon_skin']) ? $atts['icon_skin'] : 'primary';
$video_link = isset( $atts['video_link'] ) ? vc_build_link( $atts['video_link'] ) : '';
?>

  <?php if(!empty($video_link['url'])) { ?>
    <a href="<?php echo $video_link['url']; ?>" class="popup-video <?php echo esc_attr($atts['icon_as'].' '.$icon_skin); ?>">
      <?php
