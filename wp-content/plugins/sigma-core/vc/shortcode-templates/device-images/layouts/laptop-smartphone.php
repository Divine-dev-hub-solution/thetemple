...
<div class="sigma-laptop-smartphone-device-image-wrapper">
    <?php
    ...
    if ( $atts[ 'smartphone_image' ] ) {
        $image_data = wp_get_attachment_image_src( $atts[ 'smartphone_image' ], 'full' );
        $image_url  = ( isset( $image_data[0] ) && $image_data[0] ) ? $image_data[0] : '';
        if ( $image_url ) {
            ?>
            <div class="smartphone">
            <img src="<?php echo esc_url( $image_url ); ?>" alt="<?php esc_attr_e( 'Smartphone image', 'sigma-core' ) ?>">
            </div>
            <?php
        }
    }
