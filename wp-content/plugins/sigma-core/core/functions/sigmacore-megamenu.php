...
add_action( 'wp_enqueue_scripts', 'sigmacore_enqueue_megamenu_scripts', 99 );
...
/**
 * Adds the required scripts and styles to run the megamenu.
 *
 * @since 1.0.0
 */
function sigmacore_enqueue_megamenu_scripts(){

	wp_enqueue_style( 'sigma-megamenu', trailingslashit( SIGMACORE_URL ) . 'assets/css/sigma-megamenu.css', array() );
...
function sigmacore_megamenu_start_el($item_output, $item, $depth, $args){
	...
	if( $is_mm_enabled && !empty($mm_shortcode) ){
		$filter_shortcode_id = str_replace('[sigma_megamenu id="', '', $mm_shortcode );
		$get_shortcode_id = str_replace('"]', '', $filter_shortcode_id);
		$content_id = get_post($get_shortcode_id);

		if ( in_array( 'elementor/elementor.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) && ($content_id && !preg_match( '/vc_row/', $content_id->post_content )) ) {
			$description_html = '<div class="sigma_mega-menu-wrapper"><div class="sigma_mega-menu-wrapper-inner elementor-type-menu">'. Plugin::instance()->frontend->get_builder_content_for_display( $get_shortcode_id ) .'</div></div>';
		} else {
			$description_html = '<div class="sigma_mega-menu-wrapper"><div class="sigma_mega-menu-wrapper-inner">'.do_shortcode($mm_shortcode).'</div></div>';
		}
...
function sigmacore_megamenu_columns_add($columns){
	...
	return array(
	 ...
	 'title' => __('Title', 'sigma-core'),
	 'sigma_mm_shortcode_value' => __('Shortcode Value', 'sigma-core'),
	 'sigma_mm_post_id' => __( 'Megamenu ID', 'sigma-core'),
	 'date' => __( 'Date', 'sigma-core')
	);
...
function sigmacore_megamenu_shortcode( $atts = array() ) {
  ...
  return !empty($megamenu) ? $megamenu : __('No megamenu found with this ID', 'sigma-core');
}
...
function sigmacore_megamenu_vc_shortcode( $shortcodes ){
	...
	$shortcodes[] = trailingslashit( SIGMACORE_PATH ) . 'core/vc/shortcode-fields/class-sigma-vc-megamenu-tab.php';
