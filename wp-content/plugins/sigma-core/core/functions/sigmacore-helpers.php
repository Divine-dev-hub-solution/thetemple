 ...
 * @since 1.0.0
 */
function sigmacore_is_child_theme_active()
{
...
{
    return sigmacore_is_child_theme_active() ? wp_get_theme() : wp_get_theme(get_template())->get('Name');
}
...
function sigmacore_get_redux_select_options($opt_id, $has_default = true, $extras = [])
{
    ...
    } elseif($option_type == 'image_select'){
...
/**
* Get metafields dependency attributes
*
* @since 2.1.0
*/
function sigma_core_metafields_dependency_attributes( $attributes ) {
  if(! $attributes){
    return;
  }
	$atts = array();
...
/**
* filter dependency attributes output
*
* @since 2.1.0
*/
function sigma_core_filter_output( $output_attr ) {
	return $output_attr;
}
...
function sigmacore_add_icons($source = '', $icon_key = '', $splice = false, $keys = [0, 0])
{
    ...
    if (!empty($source)) {
        ...
        if (is_file($path)) {
            ...
            // Store the file contents in a string, and pass date to avoid caching issues
  ...
  global $post;
    ...
    if ($template_ids) {
          foreach ($template_ids as $id) {
            ...
            $data .= get_metadata('post', $id, '_wpb_post_custom_css', true);
