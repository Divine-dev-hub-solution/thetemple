...
/**
 * Returns all user generated page templates
 *
 * @since 2.1.1
 */
function sigmacore_get_page_template()
{

    $templates = array(0 => esc_html__('Theme Defaults (From Theme Options)', 'sigma-core'));

    $args = array(
        'post_type' => 'sigma_templates',
        'posts_per_page' => -1,
    );
    $page_template_posts = get_posts($args);

    if (is_array($page_template_posts)) {
        foreach ($page_template_posts as $template) {
