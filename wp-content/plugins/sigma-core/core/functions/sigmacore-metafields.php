 ...
 * Closing tag that will go after every metafield input
 *
 * @param string $nonce - The nonce set in the metafield callback.
 *
 * @since 1.0.0
 */
function sigmacore_check_nonce( $nonce ){
 ...
 * @var string $args [required] - An array of name and value which is responsible to show/hide fields.
 *
 * @since 1.0.0
 */
function sigmacore_custom_metafield( $args = [] ){
  ...
  if( !isset($args['name']) || empty($args['name']) ){
    return __('Set a name for the input field', 'sigma-core');
  }
  if( !isset($args['type']) || empty($args['type']) ){
    return __('Set a type for the input field', 'sigma-core');
  }
  if( !isset($args['value']) ){
    return __('Set a value for the input field', 'sigma-core');
  }
	$required_atts = array();
    $required = '';
    if (isset($args['required'])) {
        $required_atts['data-required'] = $args['required']['name'];
        $required_atts['data-value'] = $args['required']['value'];

        $required = sigma_core_metafields_dependency_attributes($required_atts);
    }
	$column_class = isset($args['column_class']) && !empty($args['column_class']) ? $args['column_class'] : '';
...
?>
	<div class="sigma_input-field-container" <?php echo sigma_core_filter_output($required); ?>>
<?php
  ...
  switch ($args['type']) {
    ...
    case 'select':
      ...
      break;
    case 'image':
      sigmacore_custom_metafield_image($args);
      break;
		case 'rating':
      sigmacore_custom_metafield_rating($args);
      break;
		case 'color':
	      sigmacore_custom_metafield_color($args);
	      break;
	  case 'date':
	      sigmacore_custom_metafield_date($args);
	      break;
		case 'time':
				sigmacore_custom_metafield_time($args);
				break;
    default:
      return __('Invalid type', 'sigma-core');
      break;
  }
	?>
	</div>
 ...
 * @since 1.0.0
 */
function sigmacore_custom_metafield_image( $args ){
  ...
  ?>
	...
	<a href="#" class="sigma_upload_image_button <?php echo esc_attr($class) ?>">
		<?php if($class == ''){
			...
			echo __('Upload Image', 'sigma-core');
		}
		?>
	</a>

  <input type="hidden" name="<?php echo esc_attr($args['name']) ?>" id="<?php echo esc_attr($args['name']) ?>" value="<?php echo esc_attr($args['value']) ?>" />
  <a href="#" class="sigma_remove_image_button components-button is-link is-destructive" style="display: <?php echo esc_attr($display) ?>"> <?php echo __('Remove Image', 'sigma-core') ?> </a>
  <?php
}
...
/**
 * The HTML for the color field
 *
 * @param string $args - @see sigmacore_custom_metafield() method
 *
 * @since 2.0.0
 */
function sigmacore_custom_metafield_color($args)
{
    ...
    ?>
    <input class="sigma_meta_color_field" type="text" id="<?php echo esc_attr($args['name']) ?>"
           name="<?php echo esc_attr($args['name']) ?>" value="<?php echo esc_attr($args['value']) ?>">
    <?php
}

/**
 * The HTML for the date input type
 *
 * @param string $args - @see sigmacore_custom_metafield() method
 *
 * @since 1.0.0
 */
function sigmacore_custom_metafield_date( $args ){

    if( $args['type'] != 'date' ){
        return;
    }
    ?>
    <input class="sigma_text date-input-field" type="text" id="<?php echo esc_attr($args['name']) ?>" name="<?php echo esc_attr($args['name']) ?>" value="<?php echo esc_attr($args['value']) ?>">
    <?php
}
/**
 * The HTML for the time input type
 *
 * @param string $args - @see sigmacore_custom_metafield() method
 *
 * @since 1.0.0
 */
function sigmacore_custom_metafield_time( $args ){

    if( $args['type'] != 'time' ){
        return;
    }
    ?>
    <input class="sigma_text time-input-field" type="text" id="<?php echo esc_attr($args['name']) ?>" name="<?php echo esc_attr($args['name']) ?>" value="<?php echo esc_attr($args['value']) ?>">
    <?php
}
